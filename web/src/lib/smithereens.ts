...
onmessage = async ({ data }) => {
    ...
    const fragmentsPath = await fetch(`/data/reference_masses/${data.fragmentsData["name"]}`);
    const fragLibrary  = await fragmentsPath.text();
    console.log(`fragmentsPath`, fragmentsPath)
    console.log(`fragLibrary`, fragLibrary);
    const muropeptidesPath = await fetch(`/data/target_structures/${data.muropeptidesData["name"]}`);
