<!-- @migration-task Error while migrating Svelte code: can't migrate `let state: PythonState = { ...defaultPythonState };` to `$state` because there's a variable named state.
